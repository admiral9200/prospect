...
*/
  ...
  async function signInWithGoogle() {
    ...
    if (error) {
      // @ts-ignore
      notification("error", "failed to Login");
    }
  }
  ...
  return (
    <>
      <Head>
        <title>Sign-in to Prosp</title>
      </Head>
      <Link href="/signup">
        {isLargeScreen && (
          <button className="absolute top-4 right-10 w-[200px] px-6 py-4 mt-4 font-semibold text-white bg-gray-900 border border-gray-200 rounded-md outline-none hover:border-violet-400 focus:outline-none">
            Sign up
          </button>
        )}
      </Link>
      <AuthLayout title="Welcome back" subtitle="Lets get these leads!">
        {notify}
        <button
        ...
        {!isLargeScreen && (
          <small className="text-base mt-2">
